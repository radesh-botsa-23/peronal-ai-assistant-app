...
const GBRAIN_ENV = { PATH: `${BUN_PATH}:/usr/bin:/bin` };
...
function runGbrain(cmd) {
  if (IS_WINDOWS) {
    ...
    return execSync(`wsl bash -c "export PATH=${BUN_PATH}:/usr/bin:/bin:\\$PATH && gbrain ${cmd}"`, {
      encoding: "utf8",
      timeout: 15000,
    });
  } else {
...
function runGbrainPut(slug, filePath) {
  ...
  if (IS_WINDOWS) {
    ...
    execSync(`wsl bash -c "export PATH=${BUN_PATH}:/usr/bin:/bin:\\$PATH && gbrain put ${slug} < '${wslPath}'"`, {
      stdio: "ignore",
      timeout: 10000,
    });
  } else {
...
export function storeDocument(slug, content) {
  ...
  const tmpFile = path.join(os.tmpdir(), `gbrain-${slug.replace(/[^a-z0-9-]/gi, "_")}.md`);
  fs.writeFileSync(tmpFile, content, "utf8");

  try {
    runGbrainPut(slug, tmpFile);
    return true;
  } catch (err) {
    console.error(`GBrain store failed for ${slug}:`, err.message);
    return false;
  } finally {
...
export function queryGBrain(query) {
  ...
  try {
    ...
    // Don't log to avoid spamming on every query failure
