 ...
 * @param {string} threadId - Thread ID of the target email
 * @param {string} replyText - Reply content
 * @returns {Promise<object>} The sent message response
 */
export async function sendEmailReply(threadId, replyText) {
  const gmail = createGmailClient();

  // 1. Get the thread details to find the last message
  ...
  // The recipient is the sender of the original email
  const recipient = originalFrom;
  
  // Format Subject: prepend "Re: " if not present
  let subject = originalSubject;
  if (!subject.toLowerCase().startsWith("re:")) {
    subject = `Re: ${subject}`;
  }

  // Construct MIME email RFC 5322 format
  const emailLines = [
    `To: ${recipient}`,
    `Subject: ${subject}`,
  ];
